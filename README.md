...
- **`jhste-pr-review`** — 사용자가 명시적으로 요청한 PR을 실제 diff 기준으로 리뷰하고 확신도 높은 코멘트를 게시합니다.
...
- `jhste-pr-review`는 사용자가 PR 코드 리뷰를 명시적으로 요청한 경우에만 사용합니다. 실제 diff를 기준으로 확신도 높은 문제만 코멘트하며 기본 review event는 `COMMENT`입니다.
