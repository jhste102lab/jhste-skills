...
`npm update -g jhste-skills`는 npm 패키지를 갱신하지만 에이전트의 skills 디렉터리에 복사한 파일까지 갱신하지는 않습니다. 0.16.0에서는 `jhste-subagent-orchestration`을 삭제합니다. 다시 복사하기만 하면 이 디렉터리와 0.15.0에서 폐기된 `jhste-pr-review`, `jhste-review-followup`, `jhste-implementation-finalizer`가 남아 계속 선택될 수 있습니다.
...
하네스에 맞게 에이전트를 새로고침하거나 재시작한 뒤 현재 Skill이 발견되고, 모든 활성 탐색 경로에서 폐기된 네 이름이 사라졌으며, 다른 스킬이 유지됐는지 확인하세요. npm 버전 확인만으로 에이전트가 새 파일을 읽었다고 판단하지 않습니다.
