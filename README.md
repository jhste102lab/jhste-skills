...
Skills are designed to be used automatically when the situation calls for them. For example, when an agent edits API code, it is guided to use `jhste-db-api-boundary`; before completion, it is guided to use `jhste-redteam`. When unsure which skill fits, route through `ask-jhste`. You can also call a skill directly, for example: `use jhste-preflight to review this change premise`, or `run jhste-redteam on this diff`.
...
| [`jhste-preflight`](skills/jhste-preflight/SKILL.md)<br>Pre-edit groundwork that verifies goal, premise, scope, boundary, failure path, and final behavior predicates before code edits | Before non-trivial code changes | Blind agreement, scope creep, unverified assumptions, unclear boundaries |
| [`jhste-change-review`](skills/jhste-change-review/SKILL.md)<br>Changed-path quality and design review that selects code-quality and architecture review cards from the touched files | After editing non-trivial code, alongside guard | Unvalidated input, silent failure, secret logging, pass-through abstraction, mixed responsibility |
...
| [`jhste-redteam`](skills/jhste-redteam/SKILL.md)<br>A read-only red-team code review skill that re-checks changed code before completion | Before declaring non-trivial code work complete | Premature “done”, missing consumer-path proof, missed null/auth/env/write/API/performance risks |
| [`jhste-workstate`](skills/jhste-workstate/SKILL.md)<br>A narrow orchestration skill for preserving work state across sessions, wait states, and durable decisions | Losing state could make multi-session, wait-state, or multi-repo work wrong, duplicated, or unsafe | Lost context, stale scratchpads, unclear approval boundaries, unsafe resume points |
...
5. Before non-trivial code changes, use `jhste-preflight` to check scope, boundary, failure path, data contract, assumptions, and the SOLID-informed review lens for changed classes/modules/functions.
6. During and after the change, use `jhste-change-review` on the changed path, and `jhste-redteam` before declaring non-trivial code work complete. Skip docs-only, comment-only, formatting-only, and trivial rename-only changes.
