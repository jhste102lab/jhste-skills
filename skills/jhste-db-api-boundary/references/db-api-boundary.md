...
When requests are user-scoped, make the auth context and owner or tenant filter visible in the seam that owns the decision. When a route mutates data, make retries, dedupe, or transaction safety visible instead of assuming one clean execution.
...
- Better: validate or narrow rows at the repository seam, map to a domain object or public DTO, and return only caller-owned fields.
