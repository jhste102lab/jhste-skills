...
2. **Run groundwork.** For non-trivial code work, use `jhste-preflight` before editing. Capture the final behavior predicates, data contracts, failure paths, and verification boundary.
...
5. **Run jhste completion checks.** When available, run `jhste-skills guard --scope changed --format text --fail-on error`. For non-trivial code changes, use `jhste-redteam` before declaring completion.
