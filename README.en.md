...
- **[`jhste-to-tickets`](skills/jhste-to-tickets/SKILL.md)** — turns defined work into one useful GitHub issue or an issue graph with real dependencies, drafting first and asking user-owned questions alongside the draft.
- **[`jhste-handoff`](skills/jhste-handoff/SKILL.md)** — creates a portable transfer for another executor or a durable record for shared evolving state, including approaches tried and set aside so the next executor does not repeat them.
