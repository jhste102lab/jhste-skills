...
- `jhste-subagent-orchestration` applies whenever the user explicitly requests subagents, delegation, multi-worker execution, or a separate-agent acceptance pass. Implicit use requires decision-complete, independently verifiable work whose consequential judgment remains with the head. It does not take over ordinary linear coding, diagnosis, prototyping, handoff authoring, finalization, PR review, or domain meanings such as Kubernetes orchestration. The harness owns worker limits and actual permission or isolation enforcement.
...
`npm test` checks package, metadata, and documentation consistency plus 88 static routing scenarios across ten covered skills. The fixture does not invoke a model or measure live automatic-trigger accuracy.
