...
- installed bridge/profile guidance tells agents to record final behavior predicates during `jhste-preflight` and run `jhste-redteam` before declaring non-trivial code work complete, while skipping docs-only, comment-only, formatting-only, and trivial rename-only changes.
