...
`cli/guard.mjs` provides the repeatable guard contract: explicit scope, text/json output with meta, fixed exit codes, baseline use/update/ratchet modes with repo-confined baseline paths, stable fingerprints without message text, trusted no-shell profile command integration, hook read-only protections, and guard failures reported separately from rule violations.
