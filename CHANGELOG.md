...
- Dropped back-compat skill aliases while keeping a migration-only replacement map for `sync`/`update`, so existing managed installs select the new replacement skills before retired directories are pruned.
...
### Validation
- `npm test` passed (12 stages). `jhste-skills guard --scope changed --format text --fail-on warning` passed with 0 warnings/errors.
