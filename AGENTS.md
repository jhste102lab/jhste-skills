...
The managed **Agent skills** block below defines the core loop (`jhste-engineering-groundwork` before non-trivial changes → guard → `jhste-red-team-review` before completion, with at most two fix + re-review cycles). Repo-local additions:

1. Keep commit-time hook behavior fast, read-only, and loop-safe.
2. Treat guard output as review evidence, not proof by itself; completion review should separate current proof, skipped/not-run checks, consumer-path proof when feasible, and residual risk.
3. Report guard runtime/config failures separately from rule violations.
