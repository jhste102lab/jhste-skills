...
export function runGlobalScenarios({ root, tmp }) {
  ...
  const opencodeFile = path.join(base, 'opencode', 'AGENTS.md');
  const cli = path.join(root, 'cli/global.mjs');
  const postinstall = path.join(root, 'cli/postinstall-global-refresh.mjs');
  ...
  run(process.execPath, [cli, '--yes', '--uninstall', '--skills-dir', skillsDir, '--claude-file', claudeFile, '--codex-file', codexFile, '--opencode-file', opencodeFile]);
  ...
  if (fs.existsSync(opencodeFile)) fail('global uninstall did not remove bridge-only opencode file');
  ...
  const invalidOpencodeFile = path.join(invalidBase, 'opencode', 'AGENTS.md');
  ...
  const invalidResult = runAny(process.execPath, [cli, '--yes', '--skills-dir', invalidSkillsDir, '--claude-file', invalidClaudeFile, '--codex-file', invalidCodexFile, '--opencode-file', invalidOpencodeFile]);
  if (invalidResult.status !== 3) fail(`global install with invalid manifest should exit 3, got ${invalidResult.status}`);
  if (fs.existsSync(invalidClaudeFile) || fs.existsSync(invalidCodexFile) || fs.existsSync(invalidOpencodeFile)) fail('global install wrote bridge files after invalid manifest');
  ...
  const conflictOpencodeFile = path.join(conflictBase, 'opencode', 'AGENTS.md');
